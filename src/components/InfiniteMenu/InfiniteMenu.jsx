/* React Bits InfiniteMenu(用户提供,原样落库;站点改造3处已标注:①canvas alpha 透背景 ②onActiveChange 回调 ③showOverlay 门控自带覆盖层) */
...
void main() {
    ...
    // 轻微动效(不改变大小:仅微漂移+亮度呼吸;缩放呼吸已按需求移除)
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    // 站点改造①:alpha:true → canvas 透明,露出下方 8K 星空背景("背景不变")
  ...
  #render() {
    ...
    // 站点改造①:预乘 alpha 混合,disc 之外透明露出星空背景
    gl.enable(gl.BLEND);
...
// 站点改造②③:onActiveChange 把聚焦项交给父组件;showOverlay 门控自带 title/description/button
export default function InfiniteMenu({ items = [], scale = 1.0, onActiveChange, showOverlay = false }) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      ...
      setActiveItem(it);
      if (onActiveChange) onActiveChange(it, itemIndex);
    };
